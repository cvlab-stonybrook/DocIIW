...
class Doc3dshadewblrefLoader(data.Dataset):
    """
    ...
    def __getitem__(self, index):
        im_name = self.files[self.split][index]                 #2/4-0_-12.5-iliad_Page_630-Nov0001-L1_4-T_15500-I_9306.png 15/849_4-hm_Page_139-ajn0001.png
        im_path = pjoin(self.root, 'img',  im_name)  
        img_foldr,fname=im_name.split('/')
        self.foldr=img_foldr
    ...
    def tight_crop(self, alb, img, wbl, sn):
        ...
        img = img*np.concatenate([msk,msk,msk], axis=-1)
        wbl = wbl*np.concatenate([msk,msk,msk], axis=-1)

        return img, wbl, msk, sn, alb
    ...
    def color_jitter(self, wb,  brightness=0, contrast=0, saturation=0, hue=0):
        ...
        hsvwb = cv2.cvtColor(wb, cv2.COLOR_RGB2HSV)
        f = random.uniform(-hue, hue)*360.
        # hsv[:,:,0] = np.clip(hsv[:,:,0] + f, 0., 360.)
        hsvwb[:,:,0] = np.clip(hsvwb[:,:,0] + f, 0., 180.)
        f = random.uniform(-saturation, saturation)*255.0
        # hsv[:,:,1] = np.clip(hsv[:,:,1] + f, 0., 255.)
        hsvwb[:,:,1] = np.clip(hsvwb[:,:,1] + f, 0., 255.)
        # im = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)
        wb = cv2.cvtColor(hsvwb, cv2.COLOR_HSV2RGB)
    ...
    def transform(self, img, wbl, alb, snorm):
        img, wbl, msk, snorm, alb=self.tight_crop(alb, img, wbl, snorm)
        # wbl= self.color_jitter(wbl, 0.2, 0.2, 0.2, 0.2)
        ...
        # folder 2 : 0.0 0.908465 0.83381 -1.0 -0.878684 -0.874137
        # folder 1 : 0.002576476, 0.90569293, 0.8339295, -0.99999994, -0.91040754, -0.91640145
        mnormx,mnormy,mnormz,nnormx,nnormy,nnormz=0.002576476, 0.908465, 0.8339295, -1.0, -0.91040754, -0.91640145
        ...
        d={'img':img, 'wbl':wbl, 'msk':msk, 'snorm':snorm, 'alb':alb}
        return d
...
if __name__ == '__main__':
    local_path = '/media/hilab/sagniksSSD/Sagnik/FoldedDocumentDataset/Doc3DShade/'
    bs = 4
    dst = Doc3dshadewblrefLoader(root=local_path, split='trainr', is_transform=True, aug=True)
